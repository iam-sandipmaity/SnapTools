...
export default defineConfig({
  ...
  resolve: {
    ...
    alias: {
      ...
      "crypto-js/md2": path.resolve(
        __dirname,
        "./src/lib/crypto-polyfills/md2.ts",
      ),
      "crypto-js/md4": path.resolve(
        __dirname,
        "./src/lib/crypto-polyfills/md4.ts",
      ),
      "crypto-js/mdc2": path.resolve(
        __dirname,
        "./src/lib/crypto-polyfills/mdc2.ts",
      ),
      "crypto-js/cast": path.resolve(
        __dirname,
        "./src/lib/crypto-polyfills/cast5.ts",
      ),
    },
  },
        ...
        manualChunks(id) {
          if (!id.includes("node_modules")) {
            ...
            if (id.includes("src/components/tools/")) {
              const toolMatch = id.match(
                /src[\/\\]components[\/\\]tools[\/\\]([^\/\\]+)[\/\\]([^\/\\]+)/,
              );
              if (toolMatch) {
                const category = toolMatch[1];
                const component = toolMatch[2].replace(
                  /\.(tsx|ts|jsx|js)$/,
                  "",
                );
                ...
                if (category === "data") {
                  ...
                  if (component === "FakeDataGenerator")
                    return "tool-data-faker";
